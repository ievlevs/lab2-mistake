...
void Cpu::dumpRegistersToFile()
{
	/*std::ofstream dumpLog("dump_registers.txt", std::ofstream::app);

	dumpLog << "Registers:\n";
	for (size_t i = 0; i < 32; ++i)
		dumpLog << "$" << i << std::setw(20) << std::hex << "0x" << m_outRegs[i] << std::endl;
	
	dumpLog << "\n//====================================================================\n\n";

	dumpLog.close();*/
	const char *mode = m_staspidor == 1 ? "w" : "a";
	FILE* dumpLog = nullptr;
	fopen_s(&dumpLog, "dump_registers.txt", mode);

	fprintf(dumpLog, "counter: %d\nRegisters:\n", m_staspidor);
	for (size_t i = 0; i < 32; ++i)
		fprintf(dumpLog,"$%d\t0x%x\n", i, m_outRegs[i]);
...
Cpu::InstructionType Cpu::decodeAndExecute(const Instruction& instruction)
{
	...
	// Dump values of registers to file after execution of each instruction
	if (m_instructionCounter < 100000) dumpRegistersToFile();
...
Cpu::InstructionType Cpu::runNextInstuction()
{
	...
	Instruction::InstructionStatus instructionStatus = m_nextInstruction.getInstructionStatus();
